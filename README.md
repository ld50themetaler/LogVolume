...
「新しく起動したゲームのオープニングが 100% 爆音で鳴るのを防ぎたい」  
...
- **全アプリ一括適用モード（起動時デフォルト）＆ 設定の自動保存**:
  ...
  - 設定したアプリ音量や消音状態、最前面表示フラグは `%APPDATA%\LogVolume\settings.json` に自動保存され、次回起動時にも微小音量設定（初期値: -34 dB / 2%）が確実に引き継がれます。
- **新着アプリの自動検知＆即時音量適用**:
...
- **クイックプリセット**:
  - `0% (消音)`, `25%`, `50%`, `75%`, `100%`
- **対数レベルメーター連動**:
  - マイクを使用するアプリ（Discord、OBS、通話ソフト等）が起動している間、入力ピークがリアルタイムに対数（dBFS）表示されます。
...
- **自動認識 & デバイス名表示**:
  - 対象デバイス（`USB audio CODEC` 等）が接続されている場合のみ有効化。

### 5. 📊 リアルタイム・オーディオピークメーター (レベルメーター)
- **聴感に即した対数（dBFS）スケールメーター（全メーター共通: マスター／アプリ／マイク）**:
  - 本ツールの最大の特徴である「微小音量（Windows標準の1%未満）」やマイクの環境ノイズ・小声を視覚的にも的確に把握できるよう、**すべてのメーターに対数（dBFS）スケール（`-60 dB` 〜 `0 dB`）を採用**。
  - `-40 dB` や `-50 dB` などの極小音量でもしっかりと LED セグメントが振れ、実際の耳で聴こえる音の強弱・ダイナミクスと完全に一致した直感的なモニタリングが可能です。
  - メーター直下には主要目盛り（`-60`, `-40`, `-30`, `-20`, `-10`, `0 dB`）および 5 dB 刻みの補助目盛り（ノッチ）を配置。
- **個別アプリセッションの正確なピーク測定**:
...
- **※マイクのレベルメーター動作について**:
  - Windows のプライバシー・省電力設計に基づき、Discord、OBS、通話アプリ、ボイスレコーダーなど、**実際にマイクを使用（録音／キャプチャ）しているアプリケーションが起動している間のみメーターが振れます**（マイクを使用しているアプリがない時は、Windows 側で音声サンプリングが休止するためメーターは動きません）。
...
- **環境**: Windows PowerShell 5.1 または PowerShell 7+（Windows 標準搭載）
...
### 通常の起動方法
1. GitHubの緑の「Code」ボタン → **「Download ZIP」** からダウンロードして解凍（または `git clone`）。
2. 解凍したフォルダ内の **`LogVolume.bat`** をダブルクリックするだけで起動します。
   - バックグラウンドで自動起動し、コマンドプロンプトの黒い画面を残さずに立ち上がります。
   - スタートアップやタスクバー・デスクトップにショートカットを作成しておくと便利です。

> [!TIP] 初回起動時の確認画面について
> 初回起動時にWindowsの保護画面（SmartScreen）が表示された場合は、「**詳細情報**」をクリックして「**実行**」を選択してください（ウイルス検知ではなく、個人開発のオープンソースファイルに対するWindows標準の初回確認です）。

### 開発・ターミナルから起動する場合
PowerShell ターミナルから直接起動することも可能です：

```powershell
powershell.exe -NoProfile -STA -ExecutionPolicy Bypass -File ".\LogVolume.ps1"
```
...
※ダウンロード直後にスクリプトがブロックされている場合は、プロパティから「許可する」にチェックを入れるか、PowerShell で `Unblock-File -Path ".\LogVolume.ps1"` を実行してください。
...
LogVolume/
├── LICENSE         # MIT License
├── LogVolume.bat   # 起動用バッチスクリプト (バックグラウンド起動)
