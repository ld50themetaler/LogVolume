...
「USBマイク（Earthworks Icon等）のダイレクトモニタリング音量を手軽に調整したい」
...
### 4. 🎧 ダイレクトモニタリング (Earthworks Icon / ハードウェアミキサー)
- **ハードウェア側サイドトーンの直接操作**:
...
- **音声を視覚的にモニタリング**:
  - マスター出力、アプリケーション、マイク入力、ダイレクトモニタリングの各セクションに、リアルタイムに振れるハードウェア風 LED セグメント方式のオーディオレベルメーターを搭載。
  - **直感的な目盛り（スケール）表示**:
    - メーター直下に `0%`, `25%`, `50%`, `75%`, `100% (0dB)` の主要目盛りと、10% 刻みの補助目盛り（ノッチ）を装備。現在のピーク音量レベルが何％・何dB付近にあるかを瞬時に直感把握できます。
  - **36 セグメント LED カラー表示**:
    - 緑（通常音量: 0〜70%）→ 黄（適正・高音量: 70〜88%）→ 赤（ピーク・クリッピング警戒: 88〜100%）。消灯時はアクリルパネル風の薄暗いセグメント輪郭が表示され、本格的なオーディオ機器の質感を演出。
- **滑らかなピークディケイ（減衰）アニメーション**:
  - 音声の急激なアタック（瞬間最大音量）に瞬時に追従し、自然で滑らかな減衰カーブを描いてフォールオフ。
- **超軽量動作（約 30 FPS）**:
